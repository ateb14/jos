...
sys_cputs(const char *s, size_t len)
{
	...
	pte_t * pte;
	uintptr_t begin = (uintptr_t) s;
	uintptr_t end = begin + len - 1;
	if(end < begin){ // overflow, it will fail the permission check then
		end = ~0;
	}
...
sys_exofork(void)
{
	...
	struct Env *env;
	int env_ret = env_alloc(&env, curenv->env_id);
	if(env_ret < 0){
		return env_ret;
	}
	env->env_status = ENV_NOT_RUNNABLE;
	env->env_tf = curenv->env_tf; // clone the general registers
	env->env_tf.tf_regs.reg_eax = 0; // clear the return value(eax)
	return env->env_id;
}
...
#define  CHECK_VA_LIMIT_ALIGN(va) ( (uintptr_t) va >= UTOP || ((uintptr_t) va) % PGSIZE != 0)
	if(CHECK_VA_LIMIT_ALIGN(va)){
		return -E_INVAL;
	} 
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	// check both virtual addresses
	if(CHECK_VA_LIMIT_ALIGN(srcva) || CHECK_VA_LIMIT_ALIGN(dstva)){
		return -E_INVAL;
	}
	// check the permission
	if((PTE_SYSCALL | perm) != PTE_SYSCALL || !(perm & PTE_U) || !(perm & PTE_P)){
	...
	if(!(pginfo = page_lookup(src_env->env_pgdir, srcva, &pte))){
		return -E_INVAL;
	}
	...
	return page_insert(dst_env->env_pgdir, pginfo, dstva, perm);
}
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
	...
	if(env->env_status != ENV_NOT_RUNNABLE || env->env_ipc_recving == 0){
	...
	if(int_va < UTOP && (uintptr_t) dstva < UTOP){
		// alignment
		if(int_va % PGSIZE != 0){
			return -E_INVAL;
		}
		...
		// share the mapping
		r = page_insert(env->env_pgdir, pginfo, dstva, perm);
		if (r < 0){
			return r;
		}
...
sys_ipc_recv(void *dstva)
{
	...
	// block the current environment and be ready for an IPC
...
syscall(uint32_t syscallno, uint32_t a1, uint32_t a2, uint32_t a3, uint32_t a4, uint32_t a5)
{
	...
	case SYS_env_set_status:
		return sys_env_set_status((envid_t) a1, (envid_t) a2);
	case SYS_page_alloc:
