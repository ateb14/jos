...
trap_init(void)
{
	...
	int hw_intps[] = {IRQ_TIMER, IRQ_KBD, IRQ_SERIAL, IRQ_SPURIOUS, IRQ_IDE, IRQ_ERROR};
	// hardware interrupts
	for(i = 0; i < sizeof(hw_intps) / sizeof(int); ++i){
		SETGATE(idt[IRQ_OFFSET + hw_intps[i]], 0, GD_KT, handlers[21 + i], 0);
	}
